...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    ...
    # The inject method takes a block that will be executed for every element in the object we called the inject method on, ie: sum + item, in first example.

    # So since its an array of 3
    # elements, the block will execute for each of the 3 elements.
    
    # The first argument of the block is the argument given to the inject method. This will be the result once its all done so think of it as creating a counter variable outside a loop that changes as you make your way through each element of the array.
    
    # The second parameter is the first element from the object the inject method  was called on. So in the array example above that would be 2. The next time the block runs the second parameter is 3, then 4, etc.
  end
